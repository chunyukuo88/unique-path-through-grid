import { uniqueGridPaths, generateGridWithObstacle } from './visualizations';
  ...
  describe('generateGridWithObstacles()', ()=>{
    describe('WHEN: Given (gridLength, gridWidth), ', ()=>{
      ...
      it('THEN: It produces a grid of that size', ()=>{
        const gridLength = generateGridWithObstacle(x, y).length;
        const gridWidth = generateGridWithObstacle(x, y)[0].length;

        expect(gridLength).toEqual(5);
        expect(gridWidth).toEqual(5);
      });
      it('AND: The grid contains an obstacle.', ()=>{
        const grid = generateGridWithObstacle(x, y);
